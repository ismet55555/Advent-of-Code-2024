...
- Numbers must ALL increase or ALL decrease
- Adjacent numbers must differ by at least 1 and at most by 3

Plan:

- Split on new line
- Loop each line
- Each number parse as i32 number and store in vec
- Get diff between numbers (negatives ok)
- Check diffs any zeros or negatives
- Check diffs for amount changed
...
- Can aremove one single value from sequence to make it safe

Plan:

- Loop through the level numbers (report) and remove each number and test for safety
- If a safe is found, then mark as safe
